...
exec consul-template -log-level err $curl_ssl -consul-addr ${CONSUL_IP}:${CONSUL_PORT} -exec-reload-signal=SIGHUP -template "/tmp/toggles.ctmpl:/etc/toggles.json" -config "/tmp/ct-config.hcl"